...
function showGame(game, $container, maxWidth, maxHeight, showmovement, isminimal, offline, seconds) {
    ...
    var zoom = 8;
    if (game.num_frames / zoom < 3)
        zoom = game.num_frames / 3;
    if (zoom < 1)
        zoom = 1;
    ...
    function animate() {
        ...
        if (!isminimal) {
            ...
            if (graphMidFrame + nf2 >= game.num_frames)
                graphMidFrame -= ((nf2 + graphMidFrame) - game.num_frames);
            else if (Math.ceil(graphMidFrame - nf2) < 0)
                graphMidFrame = nf2;
            var firstFrame = graphMidFrame - nf2, lastFrame = graphMidFrame + nf2;
            if (firstFrame < 0)
                firstFrame = 0;
            if (lastFrame >= game.num_frames)
                lastFrame = game.num_frames - 1;
            ...
            for (var a = 1; a <= game.num_players; a++) {
                for (var b = firstFrame; b <= lastFrame; b++) {
                    if (game.players[a].territories[b] > maxTer)
                        maxTer = game.players[a].territories[b] * 1.01;
                    if (game.players[a].productions[b] > maxProd)
                        maxProd = game.players[a].productions[b] * 1.01;
                    if (game.players[a].strengths[b] > maxStr)
                        maxStr = game.players[a].strengths[b] * 1.01;
                }
            }
            for (var a = 1; a <= game.num_players; a++) {
            ...
            if ((frame - firstFrame) > 0)
                graphGraphics.lineTo(GRAPH_LEFT, TER_BTM); //Deals with odd disappearing line.;
            ...
            if ((frame - firstFrame) > 0)
                graphGraphics.lineTo(GRAPH_LEFT, PROD_BTM); //Deals with odd disappearing line.;
            ...
            if ((frame - firstFrame) > 0)
                graphGraphics.lineTo(GRAPH_LEFT, STR_BTM); //Deals with odd disappearing line.;
        ...
        if (pressed[80]) { //Render productions. Don't update frames or transits. [Using p now for testing]
            ...
            for (var a = 0; a < game.height; a++) {
                ...
                for (var b = 0; b < game.width; b++) {
                    if (game.productionNormals[Math.floor(loc / game.width)][loc % game.width] < 0.33333)
                        mapGraphics.beginFill(interpolate({r: 40, g: 40, b: 40}, {r: 128, g: 80, b: 144}, game.productionNormals[Math.floor(loc / game.width)][loc % game.width] * 3));
                    else if (game.productionNormals[Math.floor(loc / game.width)][loc % game.width] < 0.66667)
                        mapGraphics.beginFill(interpolate({r: 128, g: 80, b: 144}, {r: 176, g: 48, b: 48}, game.productionNormals[Math.floor(loc / game.width)][loc % game.width] * 3 - 1));
                    else
                        mapGraphics.beginFill(interpolate({r: 176, g: 48, b: 48}, {r: 255, g: 240, b: 16}, game.productionNormals[Math.floor(loc / game.width)][loc % game.width] * 3 - 2));
            ...
            for (var a = 0; a < game.height; a++) {
                ...
                for (var b = 0; b < game.width; b++) {
                    ...
                    if (site.strength === 255)
                        mapGraphics.lineStyle(1, '0xfffff0');
        ...
        if (Math.round(xOffset) >= game.width)
            xOffset -= game.width;
        else if (Math.round(xOffset) < 0)
            xOffset += game.width;
        if (Math.round(yOffset) >= game.height)
            yOffset -= game.height;
        else if (Math.round(yOffset) < 0)
            yOffset += game.height;
